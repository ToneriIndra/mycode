...
<script type="text/javascript">
	...
	function del(id) {
		$.messager.confirm('确认信息','确认删除当前信息?',function(r){
			if(r){
	 ...
	 $(function() {
		var deltag = '${deltag}';
		if(deltag == '1') {
			$.messager.alert('提示信息','当前流程正在使用中,无法删除','warning');
		}
	}); 
</script>	
</head>
  	...
  	<table id="grid" class="easyui-datagrid">  
  		...
  		<tbody>
  				...
  				<tr>
  					...
  					<td>
  						<a onclick="window.showModalDialog('processDefinitionAction_showImg?id=${id}')" 
  						class="easyui-linkbutton" data-options="iconCls:'icon-search'">查看流程图
  						</a>
  						<a onclick="del('${id}')" 
  						class="easyui-linkbutton" data-options="iconCls:'icon-remove'">删除
  						</a>
  					</td>
  				</tr>
  				</s:iterator>
